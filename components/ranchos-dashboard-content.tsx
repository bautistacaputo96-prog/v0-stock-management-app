...
import { LayoutGrid, Clock, TrendingUp, ChevronLeft, ChevronRight, AlertTriangle, Truck } from "lucide-react"
import {
  ResponsiveContainer,
  AreaChart,
  Area,
          ...
          <div className="grid grid-cols-2 lg:grid-cols-4 gap-3 mb-6">
            <div className="bg-card rounded-lg border border-border p-4 shadow-sm">
              <div className="flex items-center gap-2 mb-1">
                ...
                <span className="text-[10px] uppercase tracking-wide text-muted-foreground">Pastones totales</span>
              </div>
              <div className="text-2xl font-bold text-foreground">{stats?.totalPastones || 0}</div>
              <div className="text-[10px] text-muted-foreground mt-0.5">Promedio: {Math.round(stats?.avgPastones || 0)}/dia</div>
            </div>
            <div className="bg-card rounded-lg border border-border p-4 shadow-sm">
              <div className="flex items-center gap-2 mb-1">
                <TrendingUp className="h-3.5 w-3.5 text-primary" />
                <span className="text-[10px] uppercase tracking-wide text-muted-foreground">Cemento total</span>
              </div>
              <div className="text-2xl font-bold text-foreground">{(stats?.totalCemento || 0).toFixed(2)} tn</div>
              <div className="text-[10px] text-muted-foreground mt-0.5">Promedio: {(stats?.avgCemento || 0).toFixed(2)} tn/dia</div>
            </div>
            <div className="bg-card rounded-lg border border-border p-4 shadow-sm">
              <div className="flex items-center gap-2 mb-1">
                <Clock className="h-3.5 w-3.5 text-destructive" />
                <span className="text-[10px] uppercase tracking-wide text-muted-foreground">Min. paradas total</span>
              </div>
              <div className="text-2xl font-bold text-foreground">{stats?.totalParadas || 0}</div>
              <div className="text-[10px] text-muted-foreground mt-0.5">Promedio: {Math.round(stats?.avgParadas || 0)} min/dia</div>
            </div>
            <div className="bg-card rounded-lg border border-border p-4 shadow-sm">
              <div className="flex items-center gap-2 mb-1">
                <LayoutGrid className="h-3.5 w-3.5 text-primary" />
                <span className="text-[10px] uppercase tracking-wide text-muted-foreground">Dias produccion</span>
              </div>
            ...
            <div className="h-64">
              <ResponsiveContainer width="100%" height="100%">
                <AreaChart data={dailyData}>
                          ...
                          {(d.cementSupplier || d.sandSupplier || d.stoneSupplier) && (
                            <div className="border-t border-border pt-1.5 mt-1 mb-1">
                              ...
                              <div className="grid grid-cols-2 gap-x-4 gap-y-0.5 text-[11px]">
                                {d.cementSupplier && <>
                                  <span className="text-muted-foreground">Cemento:</span>
                                  <span className="font-medium text-foreground">{d.cementSupplier}</span>
                                </>}
                                {d.sandSupplier && <>
                                  <span className="text-muted-foreground">Arena:</span>
                                  <span className="font-medium text-foreground">{d.sandSupplier}</span>
                                </>}
                                {d.stoneSupplier && <>
                                  <span className="text-muted-foreground">Piedra:</span>
                                  <span className="font-medium text-foreground">{d.stoneSupplier}</span>
                                </>}
                              </div>
                            </div>
                          )}
                  ...
                  {/* Supplier change vertical markers */}
